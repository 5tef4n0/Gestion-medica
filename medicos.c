...
void cargarMedicos(Medico medicos[], int *cantidad) {
	...
	*cantidad = 0;
	...
	while (fgets(linea, sizeof(linea), fp)) {
		...
		// Intentamos leer el formato nuevo (9 datos, incluyendo el precio)
		int leidos = sscanf(linea, "%[^;];%[^;];%[^;];%[^;];%d;%[^;];%[^;];%d;%f",
							m.codigo, m.cedula, m.nombre, m.especialidad, 
							&m.edad, m.horario_inicio, m.horario_fin, &m.activo, &m.precio_consulta);
		
		// LÓGICA DE COMPATIBILIDAD: Si solo leyó 8 datos (los 100 doctores antiguos)
		if (leidos == 8) {
			m.precio_consulta = 30.00; // Asignamos una tarifa base por defecto
			leidos = 9; // Validamos para que pase a la memoria
		}
...
void guardarMedicos(const Medico medicos[], int cantidad) {
	...
	for (int i = 0; i < cantidad; i++) {
		// Agregamos el %.2f al final para guardar el precio con 2 decimales
		fprintf(fp, "%s;%s;%s;%s;%d;%s;%s;%d;%.2f\n",
				medicos[i].codigo, medicos[i].cedula, medicos[i].nombre,
				medicos[i].especialidad, medicos[i].edad,
				medicos[i].horario_inicio, medicos[i].horario_fin,
				medicos[i].activo, medicos[i].precio_consulta);
	}
	fclose(fp);
}
void modificarPrecioConsulta(Medico medicos[], int n_medicos, char* codigoMedico) {
	dibujarEncabezado("MODIFICAR TARIFA DE CONSULTA");
	
	int idx = -1;
	for (int i = 0; i < n_medicos; i++) {
		if (strcmp(medicos[i].codigo, codigoMedico) == 0) {
			idx = i; break;
		}
	}
	
	if (idx == -1) { imprimirError("Medico no encontrado."); pausar(); return; }
	
	printf(COLOR_CYAN "   Tarifa actual: " COLOR_VERDE "$%.2f\n\n" COLOR_RESET, medicos[idx].precio_consulta);
	
	char buffer[20];
	float nuevoPrecio;
	do {
		printf("   > Ingrese su nueva tarifa por consulta (Ej: 45.50): $");
		leerCadena(buffer, 20);
		nuevoPrecio = atof(buffer);
		if (nuevoPrecio < 0) imprimirError("El precio no puede ser negativo.");
	} while (nuevoPrecio < 0);
	
	medicos[idx].precio_consulta = nuevoPrecio;
	barraCarga("Actualizando tarifa");
	imprimirExito("Precio actualizado correctamente.");
	pausar();
}
